...
abstract class Links
{
     ...
     */
    abstract public function outputLink(Url $url);
    ...
    /**
     * Adds the url on the list of urls to be outputted
     ...
     * @param int $priority The url's output priority (higher number means higher priority)
    ...
    public function add(string|array $urls, string $location = 'head', int $priority = 100, array $attributes = [], bool $early_hints = false, bool $preload = false) : static
    {
        ...
        foreach ($urls as $url) {
            ...
            /*if ($early_hints) {
                $this->app->response->headers->early_hints->preload->add($this->type, ['url' => $full_url, 'crossorigin' => $crossorigin]);
            }*/
    ...
    /**
     * Removes a url/urls from the list of urls to be outputted
    ...
    public function remove(string|array $urls) : static
    {
        ...
        foreach ($urls as $url) {
            //$this->early_hints->remove($full_url);
            //$this->preload->remove($full_url);
    ...
    public function output(string $location)
    {
        ...
        $this->app->plugins->run('document.links.output', $urls, $location, $this);
